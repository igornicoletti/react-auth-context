...
export const toastVariants = tv({
  slots: {
    container: 'relative pointer-events-auto min-w-96 max-w-md overflow-hidden rounded-lg bg-dracula-charcoal shadow-lg ring-1 ring-dracula-charcoal',
    message: 'flex items-start gap-3 p-4 pr-10',
    messageIcon: 'shrink-0 size-5 text-dracula-green',
    messageWrap: 'grid gap-1',
    messageTitle: 'font-semibold text-sm text-dracula-light',
    messageDesc: 'text-sm text-dracula-light/75',
    action: 'group absolute top-0 right-0 p-2.5 focus:outline-none',
    actionLabel: 'sr-only',
    actionIcon: 'size-3 text-dracula-blue group-hover:text-dracula-light',
  },
  variants: {
    color: {
      success: {
